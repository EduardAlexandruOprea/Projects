...
Value::Value(Fraction fv)
{
    if(fv.down == 0)
    {
        cerr << "Arithmetic error: Denominator is zero! ";
        Value();
        return;
    }
    isDecimal = false;
...
Value::Value(string str)
{
    ...
    if(found != string::npos)
    {
        ...
        if(left.length() >= 10)
        {
            cerr << "Arithmetic error: Number too large! ";
            *this = Value();
            return;
        }
        ...
        else if(right.length() <= 5)
        {
            ...
            {
                cerr << "Arithmetic Error: More than one '.' in a number! ";
                Value();
                return;
            }
...
Value& Value::powv(const Value z)
{
    ...
    {
        cerr << "Arithmetic error: Can't power a negative number by a non-integer number! ";
        *this = Value();
        return *this;
    }
