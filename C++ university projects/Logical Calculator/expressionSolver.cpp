...
string ExpressionSolver::solveExpression(string exp)
{
...
void ExpressionSolver::addPredefined()
{
...
string ExpressionSolver::discardSpaces(string str)
{
    string newStr = "";
...
bool ExpressionSolver::checkDeclaration(string &exp, string &newVarName, bool &isDec)
{
...
bool ExpressionSolver::groupExp(string exp)
{
...
BlockType ExpressionSolver::analyzeStrType(string str)
{
...
BlockType ExpressionSolver::charType(char c)
{
...
void ExpressionSolver::dealWithNegativeSign(string &exp)
{
...
Value ExpressionSolver::calculateExp(string exp, int startBlock, int endBlock)
{
    ...
    for(int i = endBlock-1; i >= startBlock; i--)
    {
        ...
        else if(blocks[i].type == BracR)
        {
            ...
            if(corBlock != 0 && blocks[corBlock-1].type == Func)
            {
                ...
                {
                    cerr << "Arithmetic error: Cannot square root a negative number! ";
                    return Value();
                }
        ...
        else if(blocks[i].type == Sym)
        {
            ...
            if(blockStr[0] == '*' || blockStr[0] == '/')
            {
                ...
                while(!ops.empty())
                {
                    ...
                    {
                        ops.pop();
                        Value op1 = values.top();
                        values.pop();
                        Value op2 = values.top();
                        values.pop();
...
int ExpressionSolver::findIndexOfBracketEnding(int blockId)
{
...
void ExpressionSolver::printStacks(stack<Value> values,stack<char> ops)
{
